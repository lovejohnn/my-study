...
```
static defaultProps = { initcount : 0} //React中，使用静态的defaultProps传递默认值
```
...
1. 给组件设置默认属性：static propTypes = {} 这是传一个对象，可以把外界传递过来的属性，做类型校验。

2. 给属性进行类型校验，需要先运行`cnpm i prop-types --save`( v.15.*现在已经安装好了 )

   ```
   import ReactTypes from 'prop-types' //定义了一些类型
   static propTypes = {
   	initcount: ReactTypes.number //使用 prop-types提供的类型校验
   }
   ```
